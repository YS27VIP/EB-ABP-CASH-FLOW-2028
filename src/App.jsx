...
function TemporadaForm({ empresa, fixedMarca, sbus, mode, tempState, setTempState, sinResumen, iiAuto }) {
  ...
  return (
    <>
      ...
      <div className="panel">
        <h3>Mezcla de venta por temporada — {marca} <span className="fill-badge">✏️ para llenar</span></h3>
        <div className="sub">El <b>vendedor manda el total a vender</b> de cada mes. Aquí Producto solo decide <b>de qué temporada sale ese total</b>: pon el <b>% de mezcla</b> por temporada (deben <b>sumar 100%</b> cada mes). Las <b>salidas = venta × %</b>, así siempre cuadran con lo que vende el vendedor. Si una temporada no tiene suficiente stock, se marca el <b>faltante</b> (compra más o cambia el mix). {iiAuto ? <>El <b>inventario inicial</b> viene de la matriz de arriba.</> : <>Pon también el <b>inventario inicial</b> y las <b>compras 2028</b>.</>}</div>
        <div className="tablewrap"><table className="vfix"><colgroup><col style={{ width: '260px' }} /><col style={{ width: '70px' }} />{MESES.map((_, i) => <col key={i} style={{ width: '64px' }} />)}<col style={{ width: '80px' }} /></colgroup>
          ...
          <tbody>
            <tr className="secrow"><td colSpan={15}>VENTA DEL VENDEDOR vs MEZCLA POR TEMPORADA</td></tr>
            <tr><td className="l sub2">Venta del mes (ud) <span className="unit">(la manda el vendedor)</span></td><td></td>{ventaProyMes.map((v, m) => <td key={m} className="tot">{fmt(v)}</td>)}<td className="tot">{fmt(ventaProyMes.reduce((a, b) => a + b, 0))}</td></tr>
            ...
            {SEASONS.map((s) => { const f = flujos[s]; const buy = BUY_SEASONS.includes(s); return (
              <Fragment2 key={s}>
                <tr className="secrow"><td colSpan={15}>{s}{buy ? ' · compra 2028' : ' · inventario inicial'}{iiAuto ? <span className="unit" style={{ fontWeight: 400 }}> · inicial <b style={{ color: '#a0522d' }}>{fmt(num(data[K.II(s)]))}</b> ud (de la matriz de arriba)</span> : ''}</td></tr>
    ...
    <>
      ...
      {showMatriz && <div className="panel">
          ...
          <tbody>
            {cats.map((c) => (
              <Fragment2 key={c}>
                ...
                <tr className="catrow"><td className="l">Subtotal {c} <span className="unit" style={{ fontWeight: 400 }}>(ponderado)</span></td><td className="tot">{fmt(invCat(c))}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: AUC de ${c} = promedio ponderado por unidades de sus temporadas\nDatos de origen: ${fmt(invCat(c))} ud → AUC $${money(aucPondCat(c)) || '0'}`}>{money(aucPondCat(c))}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: AUP de ${c} = promedio ponderado por unidades de sus temporadas\nDatos de origen: ${fmt(invCat(c))} ud → AUP $${money(aupPondCat(c)) || '0'}`}>{money(aupPondCat(c))}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: Margen = AUP − AUC\nDatos de origen: $${money(aupPondCat(c)) || '0'} − $${money(aucPondCat(c)) || '0'} = $${money(aupPondCat(c) - aucPondCat(c)) || '0'}`}>{money(aupPondCat(c) - aucPondCat(c))}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: Margen % = (AUP − AUC) ÷ AUP\nDatos de origen: ($${money(aupPondCat(c)) || '0'} − $${money(aucPondCat(c)) || '0'}) ÷ $${money(aupPondCat(c)) || '0'} = ${mpct(aupPondCat(c), aucPondCat(c)) || '0%'}`}>{mpct(aupPondCat(c), aucPondCat(c))}</td></tr>
              </Fragment2>
            ))}
          </tbody>
        </table></div>
        ...
        <div className="tablewrap"><table style={{ width: 'auto' }}>
          ...
          <tbody>
            {SEASONS.map((s) => { const buy = BUY_SEASONS.includes(s); const au = seasonAUP(s), ac = seasonAUC(s), u = invSeason(s); return <tr key={s}><td className="l">{s} <span className="unit" style={{ fontSize: 10 }}>{buy ? '(compra 2028)' : '(saldo anterior)'}</span></td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: unidades de ${s} = suma de las unidades de todas las categorías de esta temporada`}>{fmt(u)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: AUC de ${s} = promedio ponderado por unidades (Σ unidades×AUC por categoría ÷ unidades de ${s})\nDatos de origen: ${fmt(u)} ud → AUC $${money(ac) || '0'}`}>{money(ac)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: AUP de ${s} = promedio ponderado por unidades (Σ unidades×AUP por categoría ÷ unidades de ${s})\nDatos de origen: ${fmt(u)} ud → AUP $${money(au) || '0'}`}>{money(au)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: Margen = AUP − AUC\nDatos de origen: $${money(au) || '0'} − $${money(ac) || '0'} = $${money(au - ac) || '0'}`}>{money(au - ac)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: Margen % = (AUP − AUC) ÷ AUP\nDatos de origen: ($${money(au) || '0'} − $${money(ac) || '0'}) ÷ $${money(au) || '0'} = ${mpct(au, ac) || '0%'}`}>{mpct(au, ac)}</td></tr> })}
            <tr className="grandrow"><td className="l">TOTAL {marca}</td><td className="tot" style={{ cursor: 'help' }} title="Fórmula: total de unidades = suma de todas las temporadas y categorías">{fmt(invTot)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: AUC de la marca = promedio ponderado por unidades de todas las temporadas\nDatos de origen: ${fmt(invTot)} ud → AUC $${money(aucPondMarca) || '0'}`}>{money(aucPondMarca)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: AUP de la marca = promedio ponderado por unidades de todas las temporadas (cada temporada pesa según sus unidades)\nDatos de origen: ${fmt(invTot)} ud → AUP $${money(aupPondMarca) || '0'}`}>{money(aupPondMarca)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: Margen = AUP − AUC\nDatos de origen: $${money(aupPondMarca) || '0'} − $${money(aucPondMarca) || '0'} = $${money(aupPondMarca - aucPondMarca) || '0'}`}>{money(aupPondMarca - aucPondMarca)}</td><td className="tot" style={{ cursor: 'help' }} title={`Fórmula: Margen % = (AUP − AUC) ÷ AUP\nDatos de origen: ($${money(aupPondMarca) || '0'} − $${money(aucPondMarca) || '0'}) ÷ $${money(aupPondMarca) || '0'} = ${mpct(aupPondMarca, aucPondMarca) || '0%'}`}>{mpct(aupPondMarca, aucPondMarca)}</td></tr>
          </tbody>
        </table></div>
      </div>}
