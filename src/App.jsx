                  ...
                  {cls.map((cli) => {
                    ...
                    return (
                      <tr key={cli}>
                        ...
                        <td className={soloVer ? 'tot' : 'cell'}>{soloVer ? fmt(num(data[sk])) : <input value={data[sk] ?? ''} onChange={(e) => set(sk, e.target.value)} inputMode="decimal" style={{ width: 130 }} />}</td>
                      </tr>
                    )
                  })}
          ...
          <div className="panel">
            ...
            {isTotal && <div className="tablewrap" style={{ marginBottom: 12, maxWidth: 520 }}>
              <table style={{ width: 'auto' }}>
                <thead><tr><th className="l">Marca</th><th>Término de pago a proveedor</th></tr></thead>
                <tbody>
                  {listaM.map((mca) => <tr key={mca}><td className="l"><span style={{ display: 'inline-block', width: 9, height: 9, borderRadius: '50%', background: marcaColor(mca), marginRight: 7 }}></span>{mca}</td><td><span className="empchip" style={{ background: SP, color: '#7a4a10', marginLeft: 0 }}>{data[`PTERM|${mca}`] || '—'}</span></td></tr>)}
                </tbody>
              </table>
              <div className="sub" style={{ marginTop: 6 }}>Solo lectura: el término de cada marca a su proveedor define <b>cuándo</b> la compra se convierte en pago (Cash Out). Se edita entrando a cada marca.</div>
            </div>}
            <div className="tablewrap">
          ...
          <tbody>
            ...
            {SEASONS.map((s) => { const f = flujos[s]; const buy = BUY_SEASONS.includes(s); return (
              <Fragment2 key={s}>
                ...
                <tr><td className="l sub2">Rotación % (del saldo)</td><td></td>{MESES.map((_, m) => { const k = K.RT(s, m); const hay = ventaProyMes[m] > 0.5; return <td key={m} className={hay ? 'cell' : ''} style={hay ? undefined : { background: '#d5dbe1' }} title={hay ? '' : 'Sin ventas proyectadas este mes — no hace falta rotación aquí'}>{hay ? <input value={data[k] ?? ''} onChange={(e) => set(k, e.target.value)} inputMode="decimal" placeholder="%" /> : <input value={data[k] ?? ''} onChange={(e) => set(k, e.target.value)} inputMode="decimal" placeholder="—" style={{ background: '#c9d0d8', border: '1px solid #aab4bf', color: '#6b7480', width: 64, borderRadius: 5, padding: '6px', textAlign: 'center' }} />}</td> })}<td></td></tr>
