  ...
  return (
    <>
      ...
      <div className="panel">
        ...
        <div className="sub">El <b>vendedor manda el total a vender</b> de cada mes. Aquí Producto solo decide <b>de qué temporada sale ese total</b>: en cada mes (columna) llena el <b>% de combinación</b> hacia abajo por temporada — la fila <b>«Suma de la combinación %»</b> del final te avisa cuando llegas a <b>100%</b>. Debajo de cada temporada, <b>«Saldo por asignar»</b> te muestra cuánto stock te queda de esa temporada. Si una temporada se queda sin stock, se marca el <b>faltante</b> (compra más o cambia el mix). {iiAuto ? <>El <b>inventario inicial</b> viene de la matriz de arriba.</> : <>Pon también el <b>inventario inicial</b> y las <b>compras 2028</b>.</>}</div>
          ...
          <tbody>
            ...
            {activas.map((s) => { const f = flujos[s]; const buy = BUY_SEASONS.includes(s); const ini0 = num(data[K.II(s)]); const disponible = ini0 + rowTot(f, 'comp'); const asignado = rowTot(f, 'sal'); const restante = f[11].fin; return (
              <Fragment2 key={s}>
                ...
                <tr><td className="l" style={{ whiteSpace: 'normal', lineHeight: 1.25 }}><b style={{ color: '#0e7490', fontSize: 12.5 }}>{s}</b> <span className="unit" style={{ fontWeight: 400 }}>· % de combinación{buy ? ' (compra 2028)' : ''}</span></td>{iiAuto ? <td className="tot" style={{ color: '#a0522d', fontWeight: 700 }} title="Inventario inicial (de la matriz de arriba)">{fmt(ini0)}</td> : <td className="cell"><input value={data[K.II(s)] ?? ''} onChange={(e) => set(K.II(s), e.target.value)} inputMode="decimal" placeholder={buy ? '0' : 'inicial'} /></td>}{MESES.map((_, m) => { const k = K.RT(s, m); const hay = ventaProyMes[m] > 0.5; const pctv = num(data[k]); const pedido = Math.round(pctv / 100 * ventaProyMes[m]); const dispM = f[m].ini + f[m].comp; const over = hay && pctv > 0 && pedido > dispM; const maxPct = ventaProyMes[m] > 0 ? Math.floor(dispM / ventaProyMes[m] * 100) : 0; return <td key={m} className={hay ? 'cell' : ''} style={hay ? (over ? { background: '#ffe0e0' } : undefined) : { background: '#f1f3f6' }} title={over ? `Este ${pctv}% pide ${fmt(pedido)} ud de ${s}, pero solo hay ${fmt(dispM)} disponibles este mes. Máximo posible aquí: ~${maxPct}%. Baja el % o compra más.` : (hay ? '% de la venta de este mes que sale de esta temporada' : 'Sin ventas este mes — no hace falta mezcla aquí')}>{hay ? <input value={data[k] ?? ''} onChange={(e) => set(k, e.target.value)} onBlur={() => { const dM = f[m].ini + f[m].comp; const mp = ventaProyMes[m] > 0 ? Math.floor(dM / ventaProyMes[m] * 100) : 0; const cur = num(data[k]); if (cur > mp) { set(k, String(mp)); setMsg({ t: 'warn', x: `${s} · ${MESES[m].toUpperCase()}: bajé el % a ${mp}% — es el máximo que cubre tu stock (${fmt(dM)} ud disponibles ese mes). Si necesitas más, compra o repártelo en otra temporada.` }) } }} inputMode="decimal" placeholder="%" style={over ? { color: 'var(--bad)', fontWeight: 700 } : undefined} /> : <span style={{ display: 'inline-block', width: 64, color: '#aab2bc', textAlign: 'center', userSelect: 'none' }}>—</span>}</td> })}<td></td></tr>
            ...
            <tr className="grandrow"><td className="l" style={{ whiteSpace: 'normal', lineHeight: 1.2 }}>Suma de la combinación % <span className="unit" style={{ fontWeight: 400 }}>(cada mes debe dar 100% Y con stock)</span></td><td></td>{MESES.map((_, m) => { const hay = ventaProyMes[m] > 0.5; const sm = mixSumMes[m]; const cien = Math.abs(sm - 100) < 1; const short = ventaProyMes[m] - salidasUnits[m] > 0.5; const ok = cien && !short; return <td key={m} className="tot" style={{ fontWeight: 800, color: !hay ? 'var(--muted)' : ok ? 'var(--ok)' : 'var(--bad)' }} title={!hay ? 'Sin venta este mes' : ok ? 'Suma 100% y hay stock para cubrirlo ✓' : !cien ? 'La combinación NO suma 100% — ajusta los %' : 'Suma 100% PERO no hay inventario para cubrir ese mix — baja el % de la temporada en rojo o compra más'}>{!hay ? '—' : sm.toFixed(0) + '%'}{hay && (ok ? ' ✓' : (cien && short ? ' ⚠ sin stock' : ' ⚠'))}</td> })}<td></td></tr>
...
function ProductoTab({ empresa, usuario, sbus, fixedMarca }) {
  ...
  return (
    <>
      <div className="note ok" style={{ marginBottom: 8 }}>Producto en un solo lugar y en orden: <b>1)</b> Inventario + costo + precio por temporada y categoría · <b>2)</b> Combinación de temporadas (% por mes) — el saldo inicial se toma solo del paso 1 · <b>3)</b> Evolución mensual del AUP/AUC (consecuencia) · <b>4)</b> Compras 2028 y disponibilidad (SS28/FW28) · <b>5)</b> Resumen. Guarda cada paso con sus botones.</div>
      ...
      <div style={{ borderLeft: '4px solid #017e84', paddingLeft: 14, marginBottom: 26 }}>
        <div style={{ fontWeight: 800, color: '#017e84', fontSize: 15, marginBottom: 8 }}>Paso 2 · Combinación de temporadas (% por mes)</div>
  ...
  return (
    <>
      ...
      {showEvol && <div className="panel">
        ...
        {!MESES.some((_, m) => unitsMes(m) > 0.5) && <div className="note warn" style={{ marginBottom: 10 }}>Sale <b>vacía</b> porque aún no hay <b>salidas de inventario</b>. Ya pusiste las unidades arriba; ahora falta el <b>% de combinación</b> en el bloque de Combinación de temporadas (Paso 2). El % reparte la venta del mes por temporada y define qué se vende cada mes.</div>}
