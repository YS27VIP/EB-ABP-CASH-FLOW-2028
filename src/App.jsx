      ...
      <div className="panel">
        ...
        <div className="tablewrap"><table className="vfix"><colgroup><col style={{ width: '300px' }} /><col style={{ width: '70px' }} />{MESES.map((_, i) => <col key={i} style={{ width: '64px' }} />)}<col style={{ width: '80px' }} /></colgroup>
          ...
          <tbody>
            ...
            <tr className="grandrow"><td className="l" style={{ whiteSpace: 'normal', lineHeight: 1.2 }}>Por asignar (ud) <span className="unit" style={{ fontWeight: 400 }}>(venta − asignado; rojo = aún falta cubrir)</span></td><td></td>{MESES.map((_, m) => { const d = ventaProyMes[m] - asignMes[m]; const bad = d > 0.5; return <td key={m} className="tot" style={{ fontWeight: 800, color: !(ventaProyMes[m] > 0.5) ? 'var(--muted)' : bad ? 'var(--bad)' : 'var(--ok)' }} title={!(ventaProyMes[m] > 0.5) ? 'Sin venta este mes' : bad ? 'Todavía falta asignar unidades para cubrir la venta del mes (asigna más temporadas o compra stock)' : 'La venta del mes está cubierta ✓'}>{ventaProyMes[m] > 0.5 ? fmt(d) : '—'}{ventaProyMes[m] > 0.5 && (bad ? '' : ' ✓')}</td> })}<td className="tot">{fmt(ventaProyMes.reduce((a, b) => a + b, 0) - asignMes.reduce((a, b) => a + b, 0))}</td></tr>
            ...
            {activas.map((s) => { const f = flujos[s]; const buy = BUY_SEASONS.includes(s); const ini0 = num(data[K.II(s)]); const disponible = ini0 + rowTot(f, 'comp'); const asignado = rowTot(f, 'sal'); const restante = f[11].fin; return (
              <Fragment2 key={s}>
                ...
                <tr><td className="l" style={{ whiteSpace: 'normal', lineHeight: 1.25 }}><b style={{ color: '#0e7490', fontSize: 12.5 }}>{s}</b> <span className="unit" style={{ fontWeight: 400 }}>· unidades a rotar{buy ? ' (compra 2028)' : ''}</span></td>{iiAuto ? <td className="tot" style={{ color: '#a0522d', fontWeight: 700 }} title="Inventario inicial (de la matriz de arriba)">{fmt(ini0)}</td> : <td className="cell"><input value={data[K.II(s)] ?? ''} onChange={(e) => set(K.II(s), e.target.value)} inputMode="decimal" placeholder={buy ? '0' : 'inicial'} /></td>}{MESES.map((_, m) => { const k = K.RT(s, m); const hay = ventaProyMes[m] > 0.5; const uds = num(data[k]); const dispM = f[m].ini + f[m].comp; const over = uds > dispM + 0.5; return <td key={m} className={hay ? 'cell' : ''} style={hay ? (over ? { background: '#ffe0e0' } : undefined) : { background: '#f1f3f6' }} title={over ? `Escribiste ${fmt(uds)} ud de ${s}, pero solo hay ${fmt(dispM)} disponibles este mes. Se topó a ${fmt(dispM)}.` : (hay ? `Unidades de ${s} a vender este mes (máx ${fmt(dispM)} disponibles)` : 'Sin ventas este mes')}>{hay ? <input value={data[k] ?? ''} onChange={(e) => set(k, e.target.value)} onBlur={() => { const dM = f[m].ini + f[m].comp; const cur = num(data[k]); if (cur > dM) { set(k, String(Math.round(dM))); setMsg({ t: 'warn', x: `${s} · ${MESES[m].toUpperCase()}: bajé a ${fmt(dM)} ud — es todo el stock disponible de ${s} ese mes. Si necesitas vender más, compra o usa otra temporada.` }) } }} inputMode="decimal" placeholder="ud" style={over ? { color: 'var(--bad)', fontWeight: 700 } : undefined} /> : <span style={{ display: 'inline-block', width: 64, color: '#aab2bc', textAlign: 'center', userSelect: 'none' }}>—</span>}</td> })}<td className="tot">{fmt(rowTot(f, 'sal'))}</td></tr>
...
function ProductoTab({ empresa, usuario, sbus, fixedMarca }) {
  ...
  return (
    <>
      ...
      <div style={{ borderLeft: '4px solid #017e84', paddingLeft: 14, marginBottom: 26 }}>
        <div style={{ fontWeight: 800, color: '#017e84', fontSize: 15, marginBottom: 8 }}>Paso 2 · Combinación de temporadas (unidades por mes)</div>
