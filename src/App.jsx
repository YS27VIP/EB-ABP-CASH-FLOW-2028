...
function CashFlowForm({ role, rubro, usuario, empresa, sbus, fixedMarca }) {
  ...
  // Cuentas por cobrar de VENTA EXTERNA que se cobran en el tramo final de 2027 (OCT=0, NOV=1, DIC=2), por cliente.
  // DIC conserva la llave antigua SALDO|mca|cli (retrocompatible); OCT y NOV usan SALDO|mca|cli|0 y |1.
  const cob27Key = (mca, cli, m) => m === 2 ? `SALDO|${mca}|${cli}` : `SALDO|${mca}|${cli}|${m}`
  const cob27 = (mca, cli, m) => esInterno(mca, cli) ? 0 : num(data[cob27Key(mca, cli, m)])
  const cob27Total = (mca, m) => clientesDe(mca).reduce((s, cli) => s + cob27(mca, cli, m), 0)
  // Escalera de las cuentas por cobrar 2027: lo pendiente originado en oct(0)/nov(1)/dic(2)-27 se cobra
  // 'plazo' meses después según el término del cliente → cae en el mes de caja mi = origen + plazo (puede caer en 2028).
  const cob2027Landing = (mca, mi) => clientesDe(mca).reduce((s, cli) => { const p = CF_PLAZO_MESES[data[`TERM|${mca}|${cli}`]] ?? 0; let x = 0; for (let o = 0; o < 3; o++) { if (o + p === mi) x += cob27(mca, cli, o) } return s + x }, 0)
  // ¿En qué mes cae lo pendiente originado en el mes 'o' (0=oct,1=nov,2=dic-27) según el término?
  const caeEn = (o, term) => { const p = CF_PLAZO_MESES[term]; if (p == null) return null; const mi = o + p; return mi < CF_MESES.length ? CF_MESES[mi] : null }
  ...
  const cell = (concepto, mi) => {
    ...
    if (concepto === CASHIN) {
      const tail = isTotal ? sbuMarcas.reduce((s, m) => s + cob2027Landing(m, mi), 0) : cob2027Landing(marca, mi)  // cola de cuentas por cobrar 2027 que cae este mes (según término)
      const esc = mi < 3 ? 0 : (isTotal ? sbuMarcas.reduce((s, m) => s + getCobros(m).total[mi - 3], 0) : getCobros(marca).total[mi - 3])  // escalera de ventas 2028
      return tail + esc
    }
    if (concepto === CASH_OUT) { if (mi < 3) return 0; const j = mi - 3; return isTotal ? sbuMarcas.reduce((s, m) => s + pagosMarca(m).pagos[j], 0) : pagosMarca(marca).pagos[j] } // = pagos a proveedores (compras × término)
  ...
  const cellMarca = (mca, concepto, mi) => {
    ...
    if (concepto === CASHIN) { return cob2027Landing(mca, mi) + (mi < 3 ? 0 : getCobros(mca).total[mi - 3]) }
    if (concepto === CASH_OUT) return mi < 3 ? 0 : pagosMarca(mca).pagos[mi - 3]
  ...
  return (
    <>
      ...
      <div className="panel">
        ...
        <div className="sub">Últimos 3 meses de 2027 + proyección 2028. <b>Cash Final = Cash Inicial + <span style={{ color: '#15803d' }}>Cobros</span> − <span style={{ color: '#b91c1c' }}>Pagos</span> − <span style={{ color: '#b91c1c' }}>Costos operativos</span></b>. Lo único que llena Finanzas a mano es el <b>saldo en banco</b> (Cash Inicial de Oct-27, la celda amarilla); todo lo demás se calcula. El PSI (inventario, compras, ventas) viene de Comercial/Producto.</div>
        <div className="tablewrap">
                ...
                <Fragment2 key={gr.g}>
                  ...
                  {gr.items.map((it) => {
                    ...
                    const celdas = CF_MESES.map((_, mi) => {
                      ...
                      // Cash Inicial: el 1er mes (Oct-27) es la SEMILLA = saldo en banco (editable); el resto = Cash Final del mes anterior (calc)
                      if (it === CASH_INI) {
                        if (mi === 0 && !isTotal && !soloVer) { const k = key(marca, CASH_INI, 0); return <td key={mi} className={'cell ' + cls}><input value={data[k] ?? ''} onChange={(e) => set(k, e.target.value)} inputMode="decimal" title="Saldo en banco al inicio del plan (Oct-27)" /></td> }
                        return <td key={mi} className={'tot ' + cls} style={{ cursor: 'help' }} title={mi === 0 ? 'Saldo en banco al inicio del plan (semilla que pone Finanzas)' : 'Cash Inicial = Cash Final del mes anterior'}>{fmt(cell(it, mi))}</td>
                      }
                      ...
                      if (isTotal || esCostos || cashinCalc || cashoutCalc || comercialCalc || soloVer) {
                        const tit = brk(it, mi) || (it === CASHIN ? (mi <= DIC27 ? 'Cuentas por cobrar (venta externa) de este mes, por cliente' : 'Cobros según escalera (ventas × plazo)') : it === CASH_OUT ? 'Pagos a proveedores (compras × término de pago)' : (it === VENTAS_NETAS ? 'Unidades × AUP (Comercial)' : it === COMPRAS_FD ? 'Compras × AUC (Producto)' : (it === INV_INI || it === INV_FIN) ? 'Inventario (Producto) × AUC' : undefined))
                        const v = cell(it, mi)
                    ...
                    const fila = <tr key={it} className={esCostos ? 'catrow rowline ' + (openCostos ? 'open' : '') : undefined} onClick={esCostos ? () => setOpenCostos((o) => !o) : undefined} style={esCostos ? { cursor: 'pointer' } : undefined}><td className="l">{esCostos ? <span className="caret">▶</span> : null} {it === VENTAS_NETAS ? 'Ventas Netas Total' : it}{ayudaComp && Q(ayudaComp)}{esCostos ? <span className="unit" style={{ marginLeft: 6 }}>({openCostos ? 'ocultar' : 'ver'} detalle: {CF_COSTOS.join(' + ')})</span> : null}</td>{celdas}<td className="tot">{fmt(rowTot(it))}</td></tr>
                    if (it === CASH_FIN) {
                      const flujoCeldas = CF_MESES.map((_, mi) => { const cls = mi < 3 ? 'ya' : 'yb'; const v = flujoNeto(mi); const s = v > 0.5 ? '+ ' : v < -0.5 ? '− ' : ''; return <td key={mi} className={'tot ' + cls} style={{ color: v < -0.5 ? '#b91c1c' : v > 0.5 ? '#15803d' : undefined }}>{s}{fmt(Math.abs(v))}</td> })
      ...
      <div className="panel">
        ...
        <div className="tablewrap">
          <table className="vfix"><colgroup><col style={{ width: '210px' }} />{CF_MESES.map((_, i) => <col key={i} style={{ width: '64px' }} />)}<col style={{ width: '80px' }} /></colgroup>
            <thead><tr><th className="l">Concepto</th>{CF_M2027.map((m) => <th key={m} className="ya">{m}</th>)}{CF_M2028.map((m) => <th key={m} className="yb">{m}</th>)}<th>Total</th></tr></thead>
            <tbody>
              {[['Venta externa 2028 (base de cobros)', 'ext', '#0b5566'], ['Venta interna 2028 (incobrable · intercompañía)', 'int', '#b45309']].map(([label, pick, color]) => {
                const cs = CF_MESES.map((_, mi) => { const cls = mi < 3 ? 'ya' : 'yb'; return <td key={mi} className={'tot ' + cls} style={{ color }}>{fmt(ventaSplitMemo(pick, mi))}</td> })
      ...
      <div className="panel">
        <h3>{role.label} — Cuentas por cobrar del cierre 2027 <span className="unit">({isTotal ? `TOTAL ${sbuLbl}` : marca})</span>{!isTotal && !soloVer && <span className="fill-badge">✏️ para llenar</span>}{(isTotal || soloVer) && ESP('Espejo (solo lectura): lo captura Finanzas por marca. Aquí solo se ve.')}</h3>
        <div className="note ok" style={{ marginBottom: 12 }}>💡 <b>Cómo funciona:</b> por cada cliente, elige <b>a qué plazo te paga</b> (término) y escribe <b>lo que te quedó por cobrar</b> de sus ventas de oct, nov y dic-27. La herramienta lo cobra solo en el mes que caiga según el plazo — por ejemplo, lo de <b>octubre a 90 días entra en enero-28</b>. Eso alimenta el <b>Cash In</b> de arriba (la "cola" de 2027).</div>
        <div className="sub">{isTotal ? <>Espejo por marca. El <b>término</b> y lo <b>pendiente por mes</b> lo definió Finanzas en cada marca.</> : <>Solo la <b>venta externa</b> (los clientes internos no cuentan). Las columnas son el <b>mes en que se vendió</b>, no cuándo se cobra.</>}</div>
        {buscador}
        {isTotal ? (() => {
          const secciones = sbuMarcas.map((mca) => ({ mca, cls: clientesDe(mca).filter(matchCli) })).filter((s) => s.cls.length > 0)
          const gt = [0, 1, 2].map((m) => sbuMarcas.reduce((s, mca) => s + cob27Total(mca, m), 0))
          ...
          return (
            <div className="tablewrap">
              <table>
                <thead><tr><th className="l">Marca / Cliente</th><th>Plazo</th><th>Vendido OCT-27</th><th>Vendido NOV-27</th><th>Vendido DIC-27</th></tr></thead>
                <tbody>
                  {secciones.map(({ mca, cls }) => (
                    <Fragment2 key={mca}>
                      <tr className="sburow"><td className="l" style={{ color: marcaColor(mca) }}><span style={{ display: 'inline-block', width: 9, height: 9, borderRadius: '50%', background: marcaColor(mca), marginRight: 7 }}></span>{mca}</td><td></td>{[0, 1, 2].map((m) => <td key={m} className="tot">{fmt(cob27Total(mca, m))}</td>)}</tr>
                      {cls.map((cli) => { const tk = `TERM|${mca}|${cli}`; return <tr key={mca + '|' + cli}><td className="l sub2">{cli}</td><td>{data[tk] || '—'}</td>{[0, 1, 2].map((m) => <td key={m} className="tot">{fmt(cob27(mca, cli, m))}</td>)}</tr> })}
                    </Fragment2>
                  ))}
                  <tr className="grandrow"><td className="l">TOTAL {sbuLbl}</td><td></td>{gt.map((v, m) => <td key={m} className="tot">{fmt(v)}</td>)}</tr>
                </tbody>
              </table>
              <div className="sub" style={{ marginTop: 8 }}>🪞 Solo lectura. Estas cuentas por cobrar entran al <b>Cash In</b> en el mes que caiga según el plazo (la cola de 2027). Para editar un cliente, entra a su marca específica.</div>
            </div>
          )
        })() : (() => {
          ...
          return (
            <div className="tablewrap">
              <table>
                <thead><tr><th className="l">Cliente</th><th>¿A qué plazo paga?</th><th>Vendido OCT-27</th><th>Vendido NOV-27</th><th>Vendido DIC-27</th></tr></thead>
                <tbody>
                  {cls.length === 0 && <tr><td className="l" colSpan={5}>No hay clientes para {marca}. Carga el Histórico (2025/2026) o captura clientes en Ventas.</td></tr>}
                  {cls.map((cli) => {
                    const tk = `TERM|${marca}|${cli}`; const term = data[tk]
                    return (
                      <tr key={cli}>
                        <td className="l">{cli}</td>
                        <td>{soloVer ? (data[tk] || '—') : <select value={data[tk] ?? ''} onChange={(e) => set(tk, e.target.value)}><option value="">—</option>{CF_TERMINOS.map((t) => <option key={t}>{t}</option>)}</select>}{term && caeEn(0, term) && <div className="unit" style={{ fontSize: 10.5, marginTop: 3, color: 'var(--muted)' }}>cae en: {caeEn(0, term)} · {caeEn(1, term)} · {caeEn(2, term) || 'fuera'}</div>}</td>
                        {[0, 1, 2].map((m) => { const ck = cob27Key(marca, cli, m); return <td key={m} className={soloVer ? 'tot' : 'cell'}>{soloVer ? fmt(num(data[ck])) : <input value={data[ck] ?? ''} onChange={(e) => set(ck, e.target.value)} inputMode="decimal" style={{ width: 90 }} />}</td> })}
                      </tr>
                    )
                  })}
                  {cls.length > 0 && <tr className="grandrow"><td className="l">TOTAL</td><td></td>{[0, 1, 2].map((m) => <td key={m} className="tot">{fmt(cob27Total(marca, m))}</td>)}</tr>}
                </tbody>
              </table>
              <div className="sub" style={{ marginTop: 8 }}>Debajo del plazo verás <b>en qué mes cae</b> cada uno. Todo esto entra solo al <b>Cash In</b> de arriba (la cola de 2027); no se escribe a mano allá.</div>
            </div>
          )
        })()}
      </div>
        ...
        return (
          <div className="panel">
            ...
            {!isTotal && <div className="toolbar" style={{ marginBottom: 8 }}>
              ...
              {soloVer ? <span className="empchip" style={{ background: SP, color: '#7a4a10' }}>{data[`PTERM|${marca}`] || '—'}</span> : <select value={data[`PTERM|${marca}`] ?? ''} onChange={(e) => set(`PTERM|${marca}`, e.target.value)} style={{ background: SP }}><option value="">—</option>{CF_TERMINOS.map((t) => <option key={t}>{t}</option>)}</select>}
            </div>}
            {isTotal && <div className="tablewrap" style={{ marginBottom: 12, maxWidth: 520 }}>
              <table style={{ width: 'auto' }}>
                <thead><tr><th className="l">Marca</th><th>Término de pago a proveedor {ESP('Espejo: refleja el término que se capturó al entrar a cada marca. No se edita aquí.')}</th></tr></thead>
                <tbody>
                  {listaM.map((mca) => <tr key={mca}><td className="l"><span style={{ display: 'inline-block', width: 9, height: 9, borderRadius: '50%', background: marcaColor(mca), marginRight: 7 }}></span>{mca}</td><td><span className="empchip" style={{ background: '#eef1f4', color: '#475569', marginLeft: 0, border: '1px solid #dbe1e8' }}>{data[`PTERM|${mca}`] || '—'}</span></td></tr>)}
                </tbody>
              </table>
...
function inventarioCalc(data, marca) {
  ...
  const flujos = {}
  SEASONS.forEach((s) => { const arr = []; let saldo = g(II(s)); for (let m = 0; m < 12; m++) { const ini = saldo; const comp = g(CP(s, m)); const disp = ini + comp; const rot = g(RT(s, m)) / 100; const sal = disp * rot; const fin = disp - sal; arr.push({ ini, comp, sal, fin }); saldo = fin } flujos[s] = arr })
  ...
  return (
    <>
      ...
      <div className="panel">
        ...
        <div className="sub" style={{ marginTop: 6 }}>El costo de mantenimiento del saldo se ve en <b>Logística</b>. (Pendiente: AUP/AUC por temporada, lo conectamos luego.)</div>
      </div>
    </>
  )
}
