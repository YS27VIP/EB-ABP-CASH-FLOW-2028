  ...
  return (
    <>
      ...
      <div className="panel">
        ...
        <div className="sub">El <b>vendedor manda el total a vender</b> de cada mes. Aquí Producto solo decide <b>de qué temporada sale ese total</b>: en cada mes (columna) llena el <b>% de mezcla</b> hacia abajo por temporada — la fila <b>«Suma de la mezcla %»</b> del final te avisa cuando llegas a <b>100%</b>. Debajo de cada temporada, <b>«Saldo por asignar»</b> te muestra cuánto stock te queda de esa temporada. Si una temporada se queda sin stock, se marca el <b>faltante</b> (compra más o cambia el mix). {iiAuto ? <>El <b>inventario inicial</b> viene de la matriz de arriba.</> : <>Pon también el <b>inventario inicial</b> y las <b>compras 2028</b>.</>}</div>
          ...
          <tbody>
            ...
            {activas.map((s) => { const f = flujos[s]; const buy = BUY_SEASONS.includes(s); return (
              <Fragment2 key={s}>
                ...
                <tr className="catrow"><td className="l">Saldo por asignar (ud) <span className="unit" style={{ fontWeight: 400 }}>(lo que queda sin vender de {s})</span></td>{iiAuto ? <td className="tot">{fmt(num(data[K.II(s)]))}</td> : <td className="cell"><input value={data[K.II(s)] ?? ''} onChange={(e) => set(K.II(s), e.target.value)} inputMode="decimal" placeholder={buy ? '0' : 'inicial'} /></td>}{f.map((x, m) => { const bad = x.fin <= 0 && x.sal > 0; return <td key={m} className="tot" style={{ color: x.fin <= 0 ? 'var(--bad)' : x.fin < x.ini * 0.15 ? 'var(--warn)' : undefined, fontWeight: x.fin <= 0 ? 800 : undefined }} title={`Disponible ${fmt(x.ini + x.comp)} − vendido ${fmt(x.sal)} = queda ${fmt(x.fin)}`}>{fmt(x.fin)}</td> })}<td className="tot">{fmt(f[11].fin)}</td></tr>
              </Fragment2>) })}
            <tr className="grandrow"><td className="l" style={{ whiteSpace: 'normal', lineHeight: 1.2 }}>Suma de la mezcla % <span className="unit" style={{ fontWeight: 400 }}>(cada mes debe dar 100%)</span></td><td></td>{MESES.map((_, m) => { const hay = ventaProyMes[m] > 0.5; const sm = mixSumMes[m]; const ok = Math.abs(sm - 100) < 1; return <td key={m} className="tot" style={{ fontWeight: 800, color: !hay ? 'var(--muted)' : ok ? 'var(--ok)' : 'var(--bad)' }} title={!hay ? 'Sin venta este mes' : ok ? 'La mezcla suma 100% ✓' : 'La mezcla NO suma 100% — ajusta los %'}>{!hay ? '—' : sm.toFixed(0) + '%'}{hay && (ok ? ' ✓' : ' ⚠')}</td> })}<td></td></tr>
...
function ProductoTab({ empresa, usuario, sbus, fixedMarca }) {
  ...
  return (
    <>
      <div className="note ok" style={{ marginBottom: 8 }}>Producto en un solo lugar y en orden: <b>1)</b> Inventario + costo + precio por temporada y categoría · <b>2)</b> Mezcla de venta por temporada (% por mes) — el saldo inicial se toma solo del paso 1 · <b>3)</b> Evolución mensual del AUP/AUC (consecuencia) · <b>4)</b> Compras 2028 y disponibilidad (SS28/FW28) · <b>5)</b> Resumen. Guarda cada paso con sus botones.</div>
      ...
      <div style={{ borderLeft: '4px solid #017e84', paddingLeft: 14, marginBottom: 26 }}>
        <div style={{ fontWeight: 800, color: '#017e84', fontSize: 15, marginBottom: 8 }}>Paso 2 · Mezcla de venta por temporada (% por mes)</div>
